import 'generation_config.dart';

/// Generation 4 Configuration: Rocky Linux 9 + Asterisk 18/20 (Latest)
///
/// Timeline: 2022-Present
/// AMI Version: 3.0+
/// CDR Columns: 20+
/// Features: Full CEL, Advanced PJSIP, Multi-format recordings
class Generation4Config implements GenerationConfig {
  @override
  int get generation => 4;

  @override
  String get name => 'Latest';

  @override
  String get description => 'Rocky Linux 9 + Asterisk 18/20 (2022-Present)';

  @override
  String get osName => 'Rocky Linux';

  @override
  String get osVersion => '9.x';

  @override
  String get asteriskVersion => '18.x/20.x';

  @override
  String get pythonVersion => '3.9';

  @override
  String get cdrFilePath => '/var/log/asterisk/cdr-csv/Master.csv';

  @override
  String get recordingBasePath => '/var/spool/asterisk/monitor';

  @override
  String getRecordingPath(DateTime callDate) {
    // Generation 4: Date-based subdirectories (same as Gen 2-3)
    final year = callDate.year.toString();
    final month = callDate.month.toString().padLeft(2, '0');
    final day = callDate.day.toString().padLeft(2, '0');
    return '$recordingBasePath/$year/$month/$day';
  }

  @override
  String get pythonScriptPath => '/usr/local/bin/astrix_cdr.py';

  @override
  List<String> get supportedAmiCommands => [
    'Login',
    'Logoff',
    'SIPpeers',
    'SIPshowpeer',
    'PJSIPShowEndpoints',
    'PJSIPShowEndpoint',
    'PJSIPShowContacts',
    'PJSIPShowRegistrations', // New in Gen 4
    'PJSIPQualify', // New in Gen 4
    'QueueStatus',
    'QueueSummary',
    'QueueAdd',
    'QueueRemove',
    'QueueLog',
    'Status',
    'CoreShowChannels',
    'CoreSettings',
    'CoreStatus',
    'MeetmeList',
    'ConfbridgeList',
    'ConfbridgeListRooms',
    'CEL', // New in Gen 4
    'StasisShow', // New in Gen 4
    'Command',
  ];

  @override
  List<String> get supportedRecordingFormats => [
    'wav',
    'gsm',
    'mp3',
    'ogg',
    'opus',
  ];

  @override
  bool get supportsCoreShowChannels => true;

  @override
  bool get supportsPJSIP => true; // Full advanced support

  @override
  bool get supportsJSON => true; // Full JSON support

  @override
  bool get supportsCEL => true; // CEL enabled by default

  @override
  int get cdrColumnCount => 21; // 20+ columns

  @override
  List<String> get cdrColumns => [
    'accountcode',
    'src',
    'dst',
    'dcontext',
    'clid',
    'channel',
    'dstchannel',
    'lastapp',
    'lastdata',
    'calldate',
    'duration',
    'billsec',
    'disposition',
    'amaflags',
    'uniqueid',
    'userfield',
    'answerdate',
    'enddate',
    'sequence',
    'linkedid', // New in Gen 4
    'peeraccount', // New in Gen 4
  ];

  @override
  bool get supportsTimezone => true; // Full timezone support

  @override
  String get defaultTimezone => 'UTC';

  @override
  List<String> get supportedAuthMethods => ['publickey', 'password'];

  @override
  int get defaultSSHPort => 22;

  @override
  bool get requiresKeyAuth => true; // Key auth required

  @override
  bool get supports2FA => true; // 2FA support available

  @override
  int get defaultAMIPort => 5038;

  @override
  String get amiVersion => '3.0';

  @override
  String getPythonCommand() => 'python3.9';

  @override
  Map<String, String> getPythonScriptArgs() => {
    'cdr_command': 'cdr',
    'days_param': '--days',
    'limit_param': '--limit',
  };

  @override
  String adaptAMICommand(String command) {
    // Generation 4 supports all commands natively
    return command;
  }

  @override
  Map<String, dynamic> adaptAMIResponse(
    String command,
    Map<String, dynamic> response,
  ) {
    // Full JSON support - could convert text to JSON if needed
    // For now, return as-is
    return response;
  }

  @override
  Map<String, dynamic> adaptCDRRecord(Map<String, dynamic> record) {
    // Generation 4 has all fields, minimal adaptation needed
    return {
      ...record,
      // Ensure all fields are present with defaults
      'accountcode': record['accountcode'] ?? '',
      'src': record['src'] ?? '',
      'dst': record['dst'] ?? '',
      'dcontext': record['dcontext'] ?? '',
      'clid': record['clid'] ?? '',
      'channel': record['channel'] ?? '',
      'dstchannel': record['dstchannel'] ?? '',
      'lastapp': record['lastapp'] ?? '',
      'lastdata': record['lastdata'] ?? '',
      'calldate': record['calldate'] ?? '',
      'duration': record['duration'] ?? '',
      'billsec': record['billsec'] ?? '',
      'disposition': record['disposition'] ?? '',
      'amaflags': record['amaflags'] ?? '',
      'uniqueid': record['uniqueid'] ?? '',
      'userfield': record['userfield'] ?? '',
      'answerdate': record['answerdate'] ?? '',
      'enddate': record['enddate'] ?? '',
      'sequence': record['sequence'] ?? '',
      'linkedid': record['linkedid'] ?? record['uniqueid'] ?? '',
      'peeraccount': record['peeraccount'] ?? '',
    };
  }

  @override
  String get pythonPath => '/usr/bin/python3.9';

  @override
  List<String> get sshOptions => [
    '-o StrictHostKeyChecking=no',
    '-o UserKnownHostsFile=/dev/null',
  ];

  @override
  List<String> get supportedPythonFeatures => [
    'basic',
    'csv',
    'subprocess',
    'json',
    'pathlib',
    'asyncio',
  ];

  @override
  List<String> get systemPaths => [
    '/usr/bin',
    '/usr/local/bin',
    '/bin',
  ];

  @override
  String adaptSSHCommand(String command) {
    // Python 3.9 compatibility
    if (command.contains('python3') || command.contains('python')) {
      return command.replaceAll(RegExp(r'python3?\s'), 'python3.9 ');
    }
    return command;
  }

  @override
  Map<String, dynamic> parseAMIResponse(String response) {
    final lines = response.split('\r\n');
    final result = <String, dynamic>{};

    for (final line in lines) {
      if (line.contains(':')) {
        final parts = line.split(':');
        if (parts.length >= 2) {
          result[parts[0].trim()] = parts[1].trim();
        }
      }
    }

    return result;
  }

  @override
  String getAMILoginCommand(String username, String password) {
    return 'Action: Login\r\nUsername: $username\r\nSecret: $password\r\n\r\n';
  }

  @override
  String getAMILogoutCommand() {
    return 'Action: Logoff\r\n\r\n';
  }

  @override
  bool isAMISuccessResponse(String response) {
    return response.contains('Response: Success');
  }

  @override
  Map<String, dynamic> parseCDR(String cdrLine) {
    final values = cdrLine.replaceAll('"', '').split(',');
    final columns = cdrColumns;

    if (values.length != columns.length) {
      throw FormatException('CDR line has ${values.length} values but expected ${columns.length}');
    }

    final result = <String, dynamic>{};
    for (int i = 0; i < columns.length; i++) {
      result[columns[i]] = values[i].trim();
    }

    return result;
  }

  @override
  String formatCDR(Map<String, dynamic> cdrData) {
    final values = cdrColumns.map((col) => cdrData[col] ?? '').toList();
    return '"${values.join('","')}"';
  }

  @override
  bool isCommandSupported(String command) {
    return supportedAmiCommands.contains(command);
  }

  @override
  bool isFeatureSupported(String feature) {
    switch (feature) {
      case 'core_show_channels':
        return supportsCoreShowChannels;
      case 'pjsip':
        return supportsPJSIP;
      case 'json':
        return supportsJSON;
      case 'cel':
        return supportsCEL;
      case 'timezone':
        return supportsTimezone;
      case 'ssh_2fa':
        return supports2FA;
      default:
        return false;
    }
  }
}